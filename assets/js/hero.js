/* ==========================================================================
   ...
   · Pausa cuando la pestaña está oculta (visibilitychange).
   ...
   No emite eventos. No escucha eventos de otros módulos del sitio.
   Solo escucha DOM events estándar: animationend, click, keydown,
   pointer*, transitionend, visibilitychange.
   Ver EVENTS.md para el contrato global.

   ── LIMITACIÓN ACTUAL (review #7) ──────────────────────────────────────────
   Hace querySelector('.hero') singleton: SOLO funciona con UN .hero en la
   página. Si añades un segundo carrusel (ej. tabs estilo carrusel) no podrás
   reusar este archivo tal cual — habría que extraer un carousel(root) puro
   y wirearlo desde fuera. Ver review item #9.
...
(() => {
  ...
  // === 6) PLAY / PAUSE =======================================================
  // Recuerda si el usuario pausó manualmente, para no reanudar al volver de
  // pestaña oculta si él mismo había pausado.
  let userPaused = mqReduce.matches;
  const setPaused = (p) => {
    paused = p;
  ...
  // === 8) VISIBILITY =========================================================
  // Pausar cuando la pestaña no está visible: la animación CSS sigue
  // corriendo en segundo plano (gasta batería) si no la paramos. Al volver,
  // solo reanudamos si el usuario no había pausado manualmente.
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      setPaused(true);
    } else if (!userPaused && !mqReduce.matches) {
      setPaused(false);
    }
  });
